...
| 4 — Feature Flags reuse proof | **Done** (PR #6) | `PATCH /api/feature-flags/{id}` with strict partial body, required trimmed reason, `confirm_production`, `409 NO_CHANGE` for no-ops, guarded conditional update on observed values + `updated_at`. UI: environment-filtered list, hash-selected detail, edit form only when `can_edit`, red confirmation block gating Confirm only when `requires_confirmation`, shared error/notice/audit components. |
